...
bool b4r_httpsrv_res_cache_is_fresh(struct b4r_httpsrv_res_cache *cache) {
    ...
    if (!cache || b4r_hs_count(cache->req_headers) == 0 || b4r_hs_count(cache->res_headers) == 0)
        return false;
    modified_since = b4r_hs_get(cache->req_headers, MHD_HTTP_HEADER_IF_MODIFIED_SINCE);
    none_match = b4r_hs_get(cache->req_headers, MHD_HTTP_HEADER_IF_NONE_MATCH);
    if (b4r_is_empty(modified_since) && b4r_is_empty(none_match))
        return false;
    cc = b4r_hs_get(cache->req_headers, MHD_HTTP_HEADER_CACHE_CONTROL);
