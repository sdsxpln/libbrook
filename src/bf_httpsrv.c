...
static void _bf_httpsrv_err_va_cb(void *cls, const char *fmt, va_list va) {
    struct bf_httpsrv *srv = cls;
    if (srv->err_cb)
...
static void _bf_httpsrv_err_cb(void *cls, const char *fmt, ...) {
    va_list va;
    va_start(va, fmt);
    _bf_httpsrv_err_va_cb(cls, fmt, va);
    va_end(va);
}

struct bf_httpsrv *bf_httpsrv_new(struct bf_httpsrv_cfg *cfg, bf_httpsrv_req_cb req_cb, void *req_cls,
                                  bf_httpsrv_req_err_cb req_err_cb, void *req_err_cls,
                                  bf_httpsrv_err_cb err_cb, void *err_cls) {
    struct bf_httpsrv *srv;
    if (!cfg || !req_cb)
        return NULL;
    _B4R_NEW(&srv);
...
void bf_httpsrv_free(struct bf_httpsrv *srv) {
    if (srv) {
...
static void _bf_httpsrv_con_cb(void *cls, struct MHD_Connection *con, void **socket_ctx,
...
bool bf_httpsrv_listen(struct bf_httpsrv *srv) {
    if (!srv)
        return false;
    if (!srv->listening) {
        if (!srv->req_upld_file_prepare_cb && !srv->req_upld_file_save_cb && !srv->req_upld_file_write_cb) {
            srv->req_upld_file_prepare_cb = _bf_httpsrv_req_upld_file_prepare_cb;
            srv->req_upld_file_save_cb = _bf_httpsrv_req_upld_file_save_cb;
            srv->req_upld_file_write_cb = _bf_httpsrv_req_upld_file_write_cb;
        }
        ...
        srv->listening = (srv->mhd = MHD_start_daemon(
                ...
                &_bf_httpsrv_req_cb, srv,
#ifndef DISABLE_CON_CB
                MHD_OPTION_NOTIFY_CONNECTION, &_bf_httpsrv_con_cb, srv,
#endif
                MHD_OPTION_URI_LOG_CALLBACK, &_bf_httpsrv_req_init_cb, srv,
                MHD_OPTION_NOTIFY_COMPLETED, &_bf_httpsrv_req_fini_cb, srv,
                MHD_OPTION_EXTERNAL_LOGGER, &_bf_httpsrv_err_va_cb, srv,
                MHD_OPTION_END)) != NULL;
    }
    return srv->listening;
}

bool bf_httpsrv_listening(struct bf_httpsrv *srv) {
...
bool bf_httpsrv_shutdown(struct bf_httpsrv *srv) {
    ...
    if (srv && srv->listening) {
        ...
        if (fd != MHD_INVALID_SOCKET) {
            ...
            if (!srv->forced_shutdown && srv->shutdown_attempts > 0) {
                ...
                while (MHD_get_daemon_info(srv->mhd, MHD_DAEMON_INFO_CURRENT_CONNECTIONS)->num_connections > 0) {
                    if (shutdown_attempts >= srv->shutdown_attempts) {
                        _bf_httpsrv_err_cb(srv, S_BF_SHUTDOWN_FAILED);
                        break;
                    }
                    shutdown_attempts++;
                    sleep(1);
            ...
            _bf_httpsrv_err_cb(srv, S_BF_QUIESCE_FAILED);
...
bool bf_httpsrv_setopt(struct bf_httpsrv *srv, enum B4R_HTTPSRV_OPT opt, ...) {
    va_list va;
    if (bf_httpsrv_listening(srv))
        return false;
    va_start(va, opt);
    switch (opt) {
        case B4R_HTTPSRV_OPT_CON_CB:
#ifndef DISABLE_CON_CB
            srv->con_cb = va_arg(va, bf_httpsrv_con_cb);
            srv->con_cls = va_arg(va, void *);
            return srv->con_cb != NULL;
#else
            _B4R_LOG(srv->enabled_log, S_BF_NOT_IMPLEMENTED, _B4R_STR(BF_HTTPSRV_OPT_CON_CB));
            return false;
#endif
        case B4R_HTTPSRV_OPT_REQ_INIT_CB:
            srv->req_init_cb = va_arg(va, bf_httpsrv_req_init_cb);
            srv->req_init_cls = va_arg(va, void *);
            return srv->req_init_cb != NULL;
        case B4R_HTTPSRV_OPT_REQ_FINI_CB:
            srv->req_fini_cb = va_arg(va, bf_httpsrv_req_fini_cb);
            srv->req_fini_cls = va_arg(va, void *);
            return srv->req_fini_cb != NULL;
        case B4R_HTTPSRV_OPT_REQ_PREP_CB:
            srv->req_prep_cb = va_arg(va, bf_httpsrv_req_prep_cb);
            srv->req_prep_cls = va_arg(va, void *);
            return srv->req_prep_cb != NULL;
        case B4R_HTTPSRV_OPT_REQ_UPLD_DATA_CB:
            srv->req_upld_data_cb = va_arg(va, bf_httpsrv_req_upld_data_cb);
            srv->req_upld_data_cls = va_arg(va, void *);
            return srv->req_upld_data_cb != NULL;
        case B4R_HTTPSRV_OPT_REQ_UPLD_FILE_CBS:
            srv->req_upld_file_prepare_cb = va_arg(va, bf_httpsrv_req_upld_file_prepare_cb);
            srv->req_upld_file_prepare_cls = va_arg(va, void *);
            srv->req_upld_file_save_cb = va_arg(va, bf_httpsrv_req_upld_file_save_cb);
            srv->req_upld_file_save_cls = va_arg(va, void *);
            srv->req_upld_file_write_cb = va_arg(va, bf_httpsrv_req_upld_file_write_cb);
        ...
        case B4R_HTTPSRV_OPT_REQ_ERR_CB:
            srv->req_err_cb = va_arg(va, bf_httpsrv_req_err_cb);
        ...
        case B4R_HTTPSRV_OPT_ERR_CB:
            srv->err_cb = va_arg(va, bf_httpsrv_err_cb);
...
unsigned int bf_httpsrv_active_conns(struct bf_httpsrv *srv) {
